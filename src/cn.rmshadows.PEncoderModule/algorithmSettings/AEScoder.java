...
public class AEScoder implements IfPwdCoder {
	...
	private static byte[] encrypt(byte[] clearTextBytes, byte[] pwdBytes) {
		try {
			...
			IvParameterSpec ivSpec = new IvParameterSpec(pwdHandler(String.copyValueOf(MainProgram.keyB.getPassword())));
			// 2 获取Cipher实例
...
//			e.printStackTrace();
	...
	public String encode(String clearText) {
		...
		byte[] b = null;
		try {
			b = pwdHandler(String.copyValueOf(MainProgram.keyA.getPassword()));
...
//			System.out.println(String.copyValueOf(MainProgram.keyA.getPassword()));
		...
		try {
			...
			IvParameterSpec iv = new IvParameterSpec(pwdHandler(nKeyB));//新的KeyB
			Cipher cipher = Cipher.getInstance(CIPHER_MODE);
