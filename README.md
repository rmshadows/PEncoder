...
要求：[Java](https://www.oracle.com/index.html) 25（JDK 25）、[Maven](https://maven.apache.org/) 3.6+
...
- *PEncoder 版本：Java 25（若使用更早版本需相应调整 pom 与 module-info）。
...
- *改密码帐号等信息。格式：“平台名称⫸用户名⫸密码⫸备注”（默认分隔符为 ⫸，新建 bak 时会有「默认分隔符（可复制）：⫸」方便复制；程序仍兼容旧版冒号格式。）
...
- **执行编码/解码前自动备份到 bak 文件夹**：勾选后，在编码或解码前会把当前 bak/DB 备份到 **bak** 文件夹，文件名带时间戳，不散落在程序目录。
...
若你此前使用 v2.2，升级到 v2.3 时请注意以下变化，以便平滑迁移：

1. **数据与文件完全兼容**
   - 旧版 bak 文件（用英文冒号 `:` 分隔四列）**无需修改**，v2.3 仍可正常打开、编码、解码、改密、导出。
   - 新建 bak 时默认改为符号 `⫸` 分隔，并带一行「默认分隔符（可复制）：⫸」方便复制。你可在新建后继续用 `:` 手写，也可改用 `⫸`。

2. **若你曾在 v2.2 把 DB 解码成 bak 且发现内容不完整**
   - v2.2 存在一个 bug：解码 DB → bak 时会把解密内容的前 4 个字符截掉，导致数据丢失。
   - **处理办法**：用 v2.3 打开程序，将 **PEncoderDatabase** 与程序放在同目录，使用「文件 → 解码 DB 文件」**重新解码一次**，即可得到完整 bak；保存后再按需编码回 DB 即可。

3. **备份方式变更**
   - v2.2：退出前自动编码时会在**同目录**生成 `PEncoderDatabasebak.backup`、`PEncoderDatabase.backup`。
   - v2.3：不再在同目录生成 `.backup` 文件。改为可选「执行编码/解码前自动备份到 **bak** 文件夹」，备份文件带时间戳（如 `bak/PEncoderDatabasebak_2026-02-07_14-30-00`）。如需自动备份，请在「选项」中勾选「执行编码/解码前自动备份到 bak 文件夹」。

4. **快捷键变更**
   - 切换加密/解密模式：由 **Ctrl+M** 改为 **Alt+S**（避免与输入框冲突）。
   - 新增：**Alt+X** 执行（加密或解密）、**Alt+C** 复制输出区、**Ctrl+Q** 退出、**Ctrl+E** 编辑 bak。**Ctrl+D** 解码 DB 不变。

5. **运行环境**
   - 要求 **JDK 25**（与 v2.3 构建一致）。
...
格式示例：下面第三列（以分隔符 ⫸ 隔开算作一列）是我要加密的密码(1234567890、wodewangyiyun、skjbvinewir、rrrrrrrooo)
...
网易云⫸user@example.com⫸wodewangyiyun⫸邮箱
无⫸无⫸skjbvinewir⫸手机锁屏密码
一次性邮箱⫸example@example.com⫸rrrrrrrooo⫸有效期1年-到2020-03
```
（旧版冒号格式仍可识别）
...
- 2022.08.08——2.2
  ...
  - 新增：快捷键 Ctrl+D 解码 DB 为 bak，Alt+S 切换加密/解密模式，Alt+X 执行，Alt+C 复制输出
