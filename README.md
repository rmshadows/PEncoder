...
1. 在 **v2.2** 中：用「文件 → 解码 DB 文件」把 **PEncoderDatabase** 解密成 **PEncoderDatabasebak**（得到明文 bak）。
2. 用 **v2.3** 打开程序，把 **PEncoderDatabasebak** 和程序放在同一目录；在界面输入你 **v2.2 时用的 KeyA、KeyB**。
3. 点击「**选项 → 更换密钥**」，新密钥输入 **1234/1234**（临时密钥），确认。会生成 **PEncoderDatabasebakNEW**。
4. 把 **PEncoderDatabasebakNEW** 改名为 **PEncoderDatabasebak**（或替换原 bak），再点一次「更换密钥」，这次输入**你自己的新密码**（格式：KeyA/KeyB），确认。
5. 最后用「**文件 → 编码 bak 文件**」生成新的 **PEncoderDatabase** 即可。

**注意**：更换密钥前请确认原 bak 格式正确（四列：平台⫸账号⫸密码⫸备注，或旧版冒号分隔），否则会解密失败；若某一行报错，程序会提示第几行、平台与密文前 60 字，便于排查。
